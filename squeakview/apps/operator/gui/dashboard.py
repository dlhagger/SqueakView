...
class BehaviorDashboard(QtWidgets.QWidget):
    ...
    def _series_color(key: str, idx: int) -> str:
        ...
        return color_map.get(key, palette[idx % len(palette)])
    ...
    def _update_counts_label(self) -> None:
        ...
        for plot in self._plots:
            ...
            group = []
            for key in series:
                label = self._series_label(key).upper()
                color = self._series_color(key, series_idx)
                series_idx += 1
                group.append(
                    "<span style='white-space:nowrap;'>"
                    f"<span style='color:#9aa7cc; font-weight:600;'>{label}</span> "
                    f"<span style='color:{color}; font-weight:800;'>{self.counters.get(key, 0)}</span>"
                    "</span>"
                )
            if group:
        ...
        if parts:
            self.counts_label.setText(
                "<span style='color:#eef1ff; font-weight:800;'>EVENT COUNTS</span>"
                "&nbsp;&nbsp;&nbsp;" + "&nbsp;&nbsp;<span style='color:#46506d;'>|</span>&nbsp;&nbsp;".join(parts)
            )
        else:
            self.counts_label.setText("<span style='color:#eef1ff; font-weight:800;'>EVENT COUNTS</span>&nbsp;&nbsp;--")
