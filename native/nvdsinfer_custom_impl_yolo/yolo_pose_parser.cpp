...
bool pose_parser_debug_enabled() {
  static const bool enabled = []() {
    const char* flag = std::getenv("SQUEAKVIEW_POSE_PARSER_DEBUG");
    if (!flag) return false;
    return std::strcmp(flag, "0") != 0 &&
           std::strcmp(flag, "false") != 0 &&
           std::strcmp(flag, "False") != 0 &&
           std::strcmp(flag, "no") != 0;
  }();
  return enabled;
}
                   ...
                   float conf_thr=0.25f, float iou_thr=0.45f)
{
  ...
  if (pose_parser_debug_enabled() && !dbg_geom) {
  ...
  for (int i=0;i<num_preds;++i) {
    ...
    if (pose_parser_debug_enabled() && !debug_raw_printed) {
    ...
    if (pose_parser_debug_enabled() && !debug_det_printed) {
